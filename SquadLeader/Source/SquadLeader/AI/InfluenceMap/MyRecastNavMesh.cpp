...
void AMyRecastNavMesh::InitInfluenceMap(){
	...
	if (influenceMap) {
		...
		FNavigationQueryFilter* Filter = new FNavigationQueryFilter();
		Filter->SetFilterImplementation(dynamic_cast<const INavigationQueryFilterInterface*>(TeamOneNavFilter));
		StoreQueryFilter(FilterTeam1, MakeShareable(Filter));
	}
}