...
void UAbilityTask_PlayMontageAndWaitForEvent::OnMontageInterrupted()
{
	...
	if (AbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			if (AbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
				...
				CancelledHandle = Ability->OnGameplayAbilityCancelled.AddUObject(this, &UAbilityTask_PlayMontageAndWaitForEvent::OnMontageInterrupted);
